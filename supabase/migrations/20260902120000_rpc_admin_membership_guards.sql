-- The booking-mutation RPCs (confirm/cancel/no-show/reschedule) are SECURITY DEFINER and were
-- written before the multi-venue cutover. They still gate on the legacy pair
-- `is_admin() and court_venue(...) = current_user_venue()`, which only recognises an admin whose
-- single profiles.venue_id matches the booking. An admin who administers a venue via
-- venue_memberships (the multi-venue model) fails that check and gets "NOT_AUTHORIZED" when
-- confirming a booking there. The RLS policies were cut over in Step 7; these RPCs were missed.
--
-- Fix: a single dual-read helper — admin of the venue via venue_memberships, OR the legacy
-- profiles.role/venue_id match. Strictly more permissive than before and only in the correct
-- direction (it adds membership admins; it never removes an admin who worked before). Drop the
-- helpers once profiles.role/venue_id are retired (multi-venue Step 9).

create or replace function can_admin_venue(p_venue uuid) returns boolean
  language sql security definer stable set search_path = public as $$
    select p_venue is not null
       and (is_admin_of(p_venue) or (is_admin() and current_user_venue() = p_venue));
$$;

revoke all on function can_admin_venue(uuid) from public;
grant execute on function can_admin_venue(uuid) to authenticated, service_role;
...
begin
  if not is_admin_anywhere() and not is_admin() then raise exception 'NOT_AUTHORIZED' using errcode = 'P0001'; end if;
...
begin
  ...
  if not (
    can_admin_venue(court_venue(v_booking.court_id))
...
begin
  ...
  -- The destination court must belong to a venue the admin manages too.
  if not can_admin_venue(v_court.venue_id) then
    raise exception 'NOT_AUTHORIZED' using errcode = 'P0001';
  end if;
