-- Make the payment reference number optional for online bookings. The proof-of-payment slip
-- is still required (that's the evidence an admin verifies); the reference is now a nice-to-have
-- the customer may not always have handy. Same signature as before, so a plain create-or-replace
-- is safe. Only the PAYMENT_PROOF_REQUIRED check changes — it now requires just the slip.
...
begin
  ...
  -- Proof of payment: the slip is required for online bookings; the reference number is optional.
