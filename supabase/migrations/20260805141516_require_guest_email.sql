-- Requires an email for a guest's online booking (no account, so guest_email is the only
-- way to reach them — without it they'd never get the pending/confirmed/cancelled emails
-- added in the payment-verification and admin-action-email work). Not required for a
-- member (their account email is always used instead) or a walk-in (created by an admin
-- who already collected cash/checked payment in person — see p_source = 'walkin' below).
-- Same signature as before, so a plain create-or-replace is safe: no new/removed params,
-- unlike the earlier payment-proof migration that needed an explicit drop first.
