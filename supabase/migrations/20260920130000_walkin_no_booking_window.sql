-- Admin/front-desk walk-ins are exempt from the advance-booking window. create_booking applied the
-- OUTSIDE_BOOKING_WINDOW check to every source; walk-ins are entered by staff at the counter and may
-- be for any future date, mirroring how they already bypass the lead-time minimum. Recreated from
-- 20260918120000 with the window check gated by p_source <> 'walkin'. Same signature (replaces in
-- place); online/customer bookings are unchanged.
...
begin
  ...
  -- Walk-ins (admin/front-desk at the counter) are exempt from the advance-booking window, just
  -- as they already bypass the lead-time minimum — staff may enter a booking for any future date.
  if p_source <> 'walkin' and p_starts_at > now() + make_interval(days => v_advance_days) then
