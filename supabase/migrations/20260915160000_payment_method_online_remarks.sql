-- Collapse the two online instruments (GCash / bank transfer) into one 'online' method, and add a
-- free-text payment_remarks so the admin can note which bank and the reference number for a walk-in
-- paid online. A walk-in paid online is now settled as 'paid_online' (mirroring the online-booking
-- status), cash/complimentary stay 'paid_at_venue', and no method stays 'pay_at_venue'.
alter table bookings add column payment_remarks text;

-- Fold any existing granular values into 'online' before tightening the constraint (none in
-- production yet — this is defensive so the constraint swap can't fail on stray dev rows).
update bookings set payment_method = 'online' where payment_method in ('gcash', 'bank_transfer');

alter table bookings drop constraint if exists bookings_payment_method_check;
alter table bookings add constraint bookings_payment_method_check
  check (payment_method is null or payment_method in ('cash', 'online', 'complimentary'));

-- Recreate create_booking (from 20260915150000) with p_payment_remarks and the collapsed method
-- set. Payment status now depends on the method: online -> paid_online, cash/complimentary ->
-- paid_at_venue, none -> pay_at_venue. Online customer bookings are unchanged (awaiting_verification).
...
begin
  ...
  -- Online customer bookings await slip verification; a walk-in paid online is settled as
  -- paid_online; cash and complimentary are settled at the venue; no method means still to be paid.
  ...
  insert into bookings (
    ...
    payment_reference, payment_slip_path, payment_method, payment_remarks
  ) values (
    ...
    p_payment_method, p_payment_remarks
  )
