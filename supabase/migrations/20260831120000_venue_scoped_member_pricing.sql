-- Venue-scope member pricing. has_active_membership() checks only the profile, so a member of
-- venue A signing in on venue B's host would get B's member rate. A membership belongs to the
-- member's own venue (profiles.venue_id), so member pricing must only apply when the booker books
-- at *their* venue. Only the v_is_member line changes; the rest of create_booking is unchanged.
...
begin
  ...
  v_is_member := p_booked_by is not null and has_active_membership(p_booked_by)
    and (select venue_id from profiles where id = p_booked_by) = v_court.venue_id;
