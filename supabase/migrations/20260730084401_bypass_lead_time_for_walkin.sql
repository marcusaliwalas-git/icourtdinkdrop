-- Walk-in bookings are created by an admin standing at the venue with the customer already
-- there (see admin/calendar/actions.ts's createWalkInBooking, which is behind requireAdmin()
-- and always passes p_source = 'walkin') — the venue's min-lead-time rule exists to stop an
-- online guest from booking a slot starting in the next few minutes, which doesn't apply when
-- staff are booking what's happening right now. Skip the check for that source only; the
-- OUTSIDE_BOOKING_WINDOW (max-advance) and every other check still apply to walk-ins.
