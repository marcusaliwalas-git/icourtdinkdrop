-- Give complimentary bookings their own payment_status so they no longer read as 'paid_at_venue'
-- (which implies cash was collected). Status is display-only across the app, so this corrects the
-- representation everywhere at once. Sales exclusion stays keyed on payment_method, unchanged.
alter table bookings drop constraint if exists bookings_payment_status_check;
alter table bookings add constraint bookings_payment_status_check
  check (payment_status in (
    'pay_at_venue', 'paid_at_venue', 'awaiting_verification', 'paid_online',
    'complimentary', 'refunded', 'partially_refunded'
  ));

-- Backfill existing comps (created before this migration as paid_at_venue).
update bookings set payment_status = 'complimentary'
  where payment_method = 'complimentary' and payment_status = 'paid_at_venue';

-- Recreate create_booking (from 20260915160000) so a complimentary walk-in is stored with
-- payment_status 'complimentary' instead of 'paid_at_venue'. Everything else is unchanged.
...
begin
  ...
  -- Online customer bookings await slip verification; a walk-in paid online is paid_online; a
  -- complimentary walk-in is its own status; cash is settled at the venue; no method means still
