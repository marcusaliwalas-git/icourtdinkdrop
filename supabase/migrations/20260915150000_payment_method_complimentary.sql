-- Add 'complimentary' as a payment method for free bookings (comped court time). It's recorded
-- like any other method (so the booking shows as settled, nothing owed), but the sales page
-- excludes complimentary bookings from revenue entirely — see lib/sales.ts. The booking keeps its
-- computed price on record (what was comped), so switching it back to a paid method later doesn't
-- lose the amount.
alter table bookings drop constraint if exists bookings_payment_method_check;
alter table bookings add constraint bookings_payment_method_check
  check (payment_method is null or payment_method in ('cash', 'gcash', 'bank_transfer', 'complimentary'));

-- Recreate create_booking (from 20260915140000_booking_payment_method.sql) with 'complimentary'
-- accepted in the payment-method guard. Everything else is unchanged.
...
begin
  ...
  if p_payment_method is not null
     and p_payment_method not in ('cash', 'gcash', 'bank_transfer', 'complimentary') then
  ...
  -- Online bookings await payment verification; a walk-in with a recorded method (including a
  -- complimentary comp) is settled at the counter (paid_at_venue); a walk-in without one is still
