-- Overnight operating hours: let a venue stay open past midnight (e.g. 6 PM → 2 AM). The late
-- session belongs to the day it OPENED — Friday's 6 PM–2 AM runs as one continuous Friday
-- business day whose early-morning tail falls on Saturday's calendar date.
--
-- Model: close_time keeps its 24h `time` value, and a new `closes_next_day` flag means the close
-- lands on the following calendar day. Effective close (in minutes from the open day's midnight)
-- is close_minutes + (closes_next_day ? 1440 : 0). A booking may start late on the open day or in
-- the early morning of the next day; both are validated against the same session below.

alter table operating_hours add column closes_next_day boolean not null default false;

-- The old check forbade close <= open, which is exactly what an overnight row needs (02:00 < 18:00).
-- Keep it for same-day rows; allow anything when the close is on the next day.
alter table operating_hours drop constraint operating_hours_check;
alter table operating_hours add constraint operating_hours_close_after_open
  check (closes_next_day or close_time > open_time);

-- Recreate create_booking with an overnight-aware operating-hours check. Only that block changes;
-- everything else (lead time, closures, pricing, pending/confirmed status) is unchanged.
