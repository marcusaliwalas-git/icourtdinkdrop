-- Reverses the earlier "skip the operating-hours check when a booking crosses midnight"
-- exception. That was meant to let a long (up to 24h) booking span past closing time, but it
-- meant a careless duration pick (e.g. 3 hours from 10pm at a venue that closes at 11pm) would
-- silently bypass the operating-hours check entirely instead of being rejected. Every online
-- booking is pending-until-confirmed anyway, so that "admin reviews it" safety net wasn't
-- actually needed here — better to just reject it outright and let the guest pick a duration
-- that fits.
--
-- Every booking must now fit entirely within the operating hours of the day it STARTS on.
-- Implemented by comparing minutes-from-local-midnight without wrapping at 24h (so a booking
-- that would spill into the next calendar day naturally fails the close_time comparison,
-- rather than being detected via a separate "crosses day" branch).
