-- Add a sixth theme, "gold" (Midnight Gold) — deep navy ground with gold accents.
alter table venues drop constraint venues_theme_check;
alter table venues add constraint venues_theme_check
  check (theme in ('default', 'ocean', 'sunset', 'grape', 'light', 'gold'));

-- Recreate the super-admin theme setter with the gold option allowed.
...
begin
  ...
  if p_theme not in ('default', 'ocean', 'sunset', 'grape', 'light', 'gold') then
