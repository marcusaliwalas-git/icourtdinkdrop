-- Day-based rates: let a rate period apply only on certain weekdays (e.g. a weekend rate) in
-- addition to its time-of-day window. days_of_week is a set of Postgres DOW numbers (0 = Sunday …
-- 6 = Saturday). NULL or an empty array means "every day", so every existing rate period keeps
-- pricing exactly as before with no data migration — day-scoping only does something once an admin
-- picks specific days.
alter table court_rate_periods
  add column days_of_week smallint[]
  check (days_of_week is null or days_of_week <@ array[0, 1, 2, 3, 4, 5, 6]::smallint[]);

-- Recreated verbatim from 20260904170000_guest_phone_optional.sql, with the per-hour pricing query
-- extended to (a) skip a period whose days_of_week doesn't include the booking's weekday, and
-- (b) prefer a day-scoped period over an all-days one on days both cover (day-specific wins, the
-- same "most specific window wins" idea already used for overlapping time windows). The booking's
-- weekday (v_day_of_week, from its local start) applies to every hour, matching the time-of-day
-- logic — a booking is priced against the day it starts.
