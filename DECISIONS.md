...
## Admin review-notification email + beautified email templates (2026-08-09)

Two related asks: notify admins when a booking needs review, and make every email look branded instead of the bare `<p>`-tag plain text they'd been.

- **New `sendAdminBookingRequestEmail`, fired from `createBooking` when a booking lands `pending`.** Only online bookings are ever `pending` (walk-ins are admin-created and auto-confirmed), so this is precisely the set that needs a human to verify payment — no separate source check needed. Sent regardless of whether the *booker* gave an email; the admin notification is independent of the guest-email path. Recipients come from a new `getAdminEmails()` (`src/lib/admin-recipients.ts`) that queries `profiles` for `role = 'admin'` and resolves each to its `auth.users` email via the **service-role** client — a member/anon request session can't read other profiles (RLS) or `auth.users` at all, the same reason `adminConfirmBooking` already looks emails up this way. Sends run in `Promise.all` so multiple admins don't serialize. The email's CTA links to `/admin/bookings?status=pending` — the pending-review queue built earlier.
- **All emails now share one branded template.** Extracted a pure, dependency-free renderer into `src/lib/email-template.ts` (`renderEmail`) — deliberately *without* the `import "server-only"` that `email.ts` carries, so the template can be rendered/previewed outside the Next server runtime (which is exactly how it was visually verified this session). `email.ts` stays server-only because it holds the Resend/SMTP transport; it now just composes `renderEmail` calls. Design: table-based layout with fully inline styles (the only thing Gmail/Outlook/Apple Mail render consistently — `<style>` blocks and classes get stripped), a black header with the `iCourt·Social` wordmark, a lime (`#9fce20`) accent bar + CTA button, a light-gray details card with a monospaced reference code, and a footer. Cancellation emails swap the accent to red (`#e5484d`) across the bar, dot, and button so they read differently at a glance.
...
## Phase 3 groundwork — challenge-court rotation + organizer host access (2026-08-16)

First slice of Phase 3 (open-play sessions), built as a **migration + DB tests only — no UI yet**. It turns the dormant `sessions` / `session_signups` tables into a working court-rotation engine and, more importantly, establishes the **access model** for letting an admin hand a session to an organizer to run.

- **Two-layer access, both enforced server-side.** A `role` of `organizer` gates the *capability* (an admin sets it; the existing `guard_profile_role_change()` trigger already blocks non-admin role changes), and `sessions.host_id` scopes the *instance*. A single helper `is_session_host(session_id)` = `is_admin() OR sessions.host_id = auth.uid()` is what every rotation function checks — so an organizer can only touch their own event's courts, during its live window, never the venue calendar. Same "gate every write in a SECURITY DEFINER function" pattern as `create_booking` / `reschedule_booking`.
- **Rotation engine**: `session_start_match(session, court, team1[], team2[])` seats the next players (validates host access, that the session is live — published + inside its window — that the court is one the session holds, teams are even 1v1/2v2, players are checked-in signups of *this* session, and nobody's already on a court), `session_end_match(match, winner)` records the winner, frees the court, and stamps `last_played_at` so players rotate to the back, and `session_queue(session)` returns "next up" (checked-in, not currently playing, longest wait first). A **partial unique index** (`session_matches` one live row per court) makes double-booking a court impossible at the DB level, mirroring the booking exclusion constraint.
- **Winners-stay vs full-rotation is a UI decision, not baked in** — the engine just stamps who played; the host screen chooses whether to re-seat the winners or let them rotate. Streaks/match-history are derivable from `session_matches` + `session_match_players` and left as a read to add when the host screen needs it.
- **Test note worth remembering**: Postgres freezes `now()` for a whole transaction, so inside a single `withRollback` test a just-played player's rotation stamp equals everyone's check-in time and queue *ordering* can't be observed — the fair-ordering test gives the never-played player an explicit earlier check-in, and otherwise asserts rotation via the stamped `last_played_at` rather than in-transaction time ordering. 10 DB tests cover the happy paths and every rejection (non-host, court-not-in-session, court-in-use, already-playing, not-checked-in, bad team size, session-not-live); full suite green.
- **Not done here (the rest of this slice)**: session CRUD + host-assignment UI, a `requireSessionHost` route guard, the `/host/sessions/[id]` run screen, check-in, and realtime board updates. And the migration is applied/tested **locally only** — deliberately *not* pushed to production, since it's inert groundwork with no app surface yet.

## Phase 3 host UI — session management + the run board (2026-08-16)

Built the application surface on top of that groundwork: admins create and assign sessions, and the assigned organizer runs court rotation from a live board. A second migration (`20260816160000_session_host_ops`) adds the roster writes the UI needs and the RLS/realtime wiring so a **non-admin** organizer can actually operate.

- **Assigning a host IS the grant.** There's no separate "make this person an organizer" screen — when an admin picks a host on a session, `upsertSession` bumps a plain `player` up to `organizer` (only ever raises, never demotes; the role guard permits it because it runs in an admin session). One action grants the capability *and* scopes the instance, which is exactly the two-layer model made concrete. An admin can still assign an existing organizer/admin without side effects.
- **`requireSessionHost` leans on RLS instead of re-deriving authz.** New `sessions_host_select` (`host_id = auth.uid()`) plus the existing `*_admin_all` policies mean a session row is readable by *exactly* its host and admins — so the guard's rule is simply "select the row; if it comes back you're authorized." No role branch to keep in sync with the DB. Same for `/host` (lists your own sessions) and the run board's reads. Verified live across all three cases: admin (full), the assigned organizer (her session only, blocked from `/admin`), and a *different* organizer (empty `/host`, redirected off a session he doesn't host).
- **The run board is `router.refresh()`-driven, not client-state.** Every write (`session_start_match`, `session_end_match`, check-in, add/remove walk-in) is a thin server action over the SECURITY DEFINER functions; on success the client refreshes and re-renders authoritative server data. No optimistic duplication of the rotation state on the client to drift out of sync. Seat buttons stay disabled during a pending transition so seating two courts back-to-back can't double-assign the front of the queue.
- **Realtime is a nudge, with a poll as the floor.** The board subscribes to `session_matches` / `session_match_players` / `session_signups` (added to the `supabase_realtime` publication, guarded so re-runs are a no-op) and calls `router.refresh()` on any change — realtime honours RLS, so a change is only delivered to someone who can already read the row. A 12s interval is the fallback if the socket isn't available, so the board is never staler than that even without realtime. Match timers tick from a local clock, independent of refreshes.
- **Seating auto-pulls from the queue.** A court offers Singles (next 2) / Doubles (next 4) straight off the front of the wait order; ending a match asks who won and rotates everyone who played to the back (`last_played_at`). This is honest **full rotation** for v1 — winners-stay and manual partner-picking are the deliberate next iteration, and the engine already records enough (`session_match_players` + winner) to add them without a schema change. Roster adds are walk-ins by name, the real challenge-court desk flow.
- **Verified end-to-end live**: created & published a session (datetime-local correctly resolved to the venue's Manila zone), added a roster, seated a doubles match (running timer), ended it with a winner, and watched the four who played rotate behind the never-played player — then added a walk-in from the board. 7 new DB tests for the roster functions (add / check-in toggle / remove, each with its not-authorized and already-playing rejections); full suite **78 green**, `tsc` + `eslint` clean.
