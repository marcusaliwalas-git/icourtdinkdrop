...
- **`reschedule_booking(booking_id, new_court_id, new_starts_at)`** (migration `20260811155321`) — a SECURITY DEFINER function mirroring `create_booking`'s validation for the destination slot: keeps the original **duration**, enforces operating hours, closures, and no-double-booking (explicit conflict check plus the exclusion constraint as backstop), and bypasses only the min-lead-time rule (an admin deliberately moving a booking, same exemption walk-ins get). It recomputes the price for the new slot on the new court with the same per-hour rate-period logic, then sets the booking's total to **`greatest(old, new)`**: a pricier destination bills the higher new price (the admin collects the difference in person), a cheaper one keeps the original paid amount and forfeits the difference. Either way the recorded total never drops — matching the no-refund rule, and keeping realized revenue monotonic. Moves both `bookings.time_range`/`court_id` and the `booking_slots` mirror, and writes a `booking_rescheduled` audit entry.
- **Confirmed the money policy with the owner before building** (three decisions): who reschedules → **admin only**; a cheaper destination → **forfeit the difference** (no refund, no credit system); a pricier destination → admin **records it settled** (collects in person, no re-verification round-trip). These are exactly what the `greatest(old,new)` rule + admin-only gating implement.
- **UI** is an inline mode inside the existing `BookingActionSheet` (a new `RescheduleForm`), not a second stacked sheet — a second `<Sheet>` would unmount the moment the calendar cleared its selection. A "Reschedule" button (shown only for not-yet-started bookings, alongside Cancel) switches the sheet to a native date picker + a start-time `<select>` built from the venue's operating hours for the chosen day, with a **live price preview** (via `lib/pricing`, which already mirrors the SQL pricing) showing the settle-up amount to collect or the forfeited difference. A new `getRescheduleContext` action returns everything the preview needs (duration, current total, rate periods, membership, operating hours) in one round-trip so the preview updates instantly without a call per slot; the authoritative price is still recomputed server-side in the function.
