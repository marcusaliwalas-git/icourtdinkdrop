...
## Sales / revenue admin tab (2026-08-12)

Added a `/admin/sales` "Core v1" revenue view (scope confirmed with the owner), reusing the Week/Month/Year/Custom period picker from Top Customers.

- **What counts as revenue was the load-bearing decision** (the no-refund / full-payment policy makes it cleaner than a typical POS): **realized revenue = `confirmed` + `completed` + `no_show`** bookings summed by `total_cents`. No-shows are included because under the no-refund policy the money was kept, so excluding them would understate real sales — the page states this rule explicitly so the number is never mistaken for something narrower. `pending` (awaiting payment verification) is surfaced as its own stat, never mixed into realized. `cancelled` counts as zero. A rescheduled booking is inherently counted once, at its final `greatest(old,new)` total, since a reschedule mutates the row in place rather than creating a second one.
- **The aggregation is a pure function** (`src/lib/sales.ts` `summarizeSales`), so the revenue rules live in one unit-tested place (`tests/unit/sales.test.ts`, 8 cases) rather than being buried in the page — same pattern as `pricing.ts` / `home-status.ts` / `period-range.ts`. The page fetches bookings for the range, precomputes each row's venue-local ISO weekday, and hands plain rows to the function; the function does the realized/awaiting split and the by-court / by-source / by-weekday breakdowns. Weekday breakdown is ordered Mon→Sun (chronological) rather than by revenue so the week reads naturally; court/source are sorted by revenue.
- **Headline stats**: realized revenue with a **% vs the previous comparable window** (prior week/month/year, or an equal-length window immediately before a custom range) — computed by running the same summary over the previous range; the delta is hidden when the prior window had zero revenue (avoids a meaningless "+∞%"). Plus booking count, average booking value, and the separate awaiting-verification total.
- **Deliberately deferred from v1** (matching the chosen scope): the money-to-collect panel (pay-at-venue owed), the losses panel (cancellation/no-show counts), a revenue-over-time chart, and CSV export. Also skipped a true peak/off-peak split — "peak" is per-court rate-period data with no owner-defined label, so day-of-week is the honest time cut for now.
- **Verified live**: Month view showed ₱5,400 realized across 4 bookings with a −41% vs-previous badge, ₱29,500 awaiting (23 pending) held separate, and all three breakdowns reconciling to the ₱5,400 total; Year view aggregated 12 bookings (₱14,600) and correctly showed no vs-previous badge (2025 has no data). `tsc`, `eslint`, and all 77 tests pass.
